...
const getTierAccent = (rank: number) => {
  ...
  if (rank <= 30) return { label: 'Silver', color: '#475569' };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <h2 className="text-xl font-bold tracking-widest uppercase" style={{ color: '#0f766e' }}>Faction HQ</h2>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl px-4 py-3" style={{ background: '#f1f5f9', border: '1px solid #1d4ed840' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#60a5fab0' }}>Allied Factions</p><p className="mt-2 text-2xl font-semibold" style={{ color: '#0f172a' }}>{alliedCount}</p><p className="text-xs mt-1" style={{ color: '#6b7280' }}>Safe-route perks online</p></div>
          <div className="rounded-xl px-4 py-3" style={{ background: '#f1f5f9', border: '1px solid #f9731640' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#fdba74' }}>Claimable Rewards</p><p className="mt-2 text-2xl font-semibold" style={{ color: '#0f172a' }}>{claimableRewards}</p><p className="text-xs mt-1" style={{ color: '#6b7280' }}>Milestones ready to cash out</p></div>
          <div className="rounded-xl px-4 py-3" style={{ background: '#f1f5f955', border: '1px solid #22c55e40' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#86efac' }}>Weekly Reset</p><p className="mt-2 text-2xl font-semibold" style={{ color: '#0f172a' }}>{formatResetLabel(nextWeeklyReset)}</p><p className="text-xs mt-1" style={{ color: '#6b7280' }}>Sunday ladder payout window</p></div>
        </div>
      </div>

      <section className="space-y-3" aria-label="faction-roster">
        <div><h3 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Faction Roster</h3><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Unlocked contacts, chain progress, and the factions still watching from the sidelines.</p></div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {roster.map(({ factionId, definition, standing, tier, unlocked, completedChains, chainCount, activeArc, activeProgress }) => (
            <div key={factionId} className="rounded-xl p-4" style={{ background: unlocked ? '#f8fafc' : '#0b0b0b', border: `1px solid ${unlocked ? `${definition.color}55` : '#cbd5e1'}`, opacity: unlocked ? 1 : 0.78 }}>
              <div className="flex items-start justify-between gap-3"><div><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: `${definition.color}cc` }}>{definition.label}</p><p className="mt-2 text-lg font-semibold" style={{ color: '#0f172a' }}>{tier}</p></div><span className="rounded-full px-2 py-1 text-[10px] uppercase tracking-widest" style={{ background: unlocked ? `${definition.color}18` : '#f1f5f9', color: unlocked ? definition.color : '#6b7280' }}>{unlocked ? 'Unlocked' : 'Locked'}</span></div>
              <p className="mt-3 text-xs leading-5" style={{ color: '#94a3b8' }}>{definition.summary}</p>
              <div className="mt-4 space-y-2 text-xs"><div className="flex items-center justify-between"><span style={{ color: '#6b7280' }}>Rep</span><span style={{ color: definition.color }}>{standing > 0 ? `+${standing}` : standing}</span></div><div className="flex items-center justify-between"><span style={{ color: '#6b7280' }}>Chain Status</span><span style={{ color: '#475569' }}>{chainCount > 0 ? `${completedChains}/${chainCount}` : 'Intel only'}</span></div></div>
              <div className="mt-4 rounded-lg p-3" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#6b7280' }}>Current Arc</p><p className="mt-2 text-sm font-medium" style={{ color: '#1e293b' }}>{activeArc}</p><p className="mt-1 text-xs" style={{ color: '#9ca3af' }}>{activeProgress}</p><p className="mt-3 text-[11px]" style={{ color: '#6b7280' }}>{unlocked ? definition.currentArc : definition.unlockHint}</p></div>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="grid gap-6 2xl:grid-cols-[minmax(0,2fr)_minmax(340px,0.95fr)]">
        <section className="space-y-4" aria-label="questgiver-details">
          <div><h3 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Questgiver Briefings</h3><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Portrait-led HQ cards with perk previews, contract hooks, and milestone claim buttons.</p></div>
              ...
              return (
                <div key={factionId} className="overflow-hidden rounded-2xl" style={{ background: '#0b0b0b', border: `1px solid ${definition.color}55` }}>
                  <div className="grid gap-0 lg:grid-cols-[280px_minmax(0,1fr)]">
                    <div className="relative min-h-[280px] overflow-hidden" style={{ background: '#020617' }}><img src={questgiver.portraitPath} alt={questgiver.name} className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, rgba(0,0,0,0.12) 0%, rgba(0,0,0,0.82) 78%)' }} /><div className="relative z-10 flex h-full flex-col justify-end p-5"><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: `${definition.color}e0` }}>{definition.label}</p><h3 className="mt-2 text-2xl font-semibold" style={{ color: '#0f172a' }}>{questgiver.name}</h3><p className="mt-1 text-sm" style={{ color: '#334155' }}>{questgiver.title}</p></div></div>
                    <div className="p-5 space-y-4">
                      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.9fr)]"><div><p className="text-sm leading-6" style={{ color: '#475569' }}>{questgiver.summary}</p><p className="mt-3 text-sm leading-6" style={{ color: '#94a3b8' }}>{definition.currentArc}</p><div className="mt-4 flex flex-wrap gap-2">{definition.contractHooks.map((hook) => <span key={hook} className="rounded-full px-3 py-1 text-[11px] uppercase tracking-widest" style={{ background: `${definition.color}16`, color: definition.color }}>{hook}</span>)}</div></div><div className="rounded-xl p-4" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#6b7280' }}>Perk Preview</p><p className="mt-3 text-lg font-semibold" style={{ color: definition.color }}>{standing > 0 ? `+${standing}` : standing} rep</p><p className="mt-1 text-xs uppercase tracking-widest" style={{ color: '#1e293b' }}>{formatFactionTier(standing)}</p><div className="mt-4 space-y-2 text-xs">{questgiver.highRepRewards.map((reward) => <p key={reward} style={{ color: '#475569' }}>{reward}</p>)}</div></div></div>
                      <div className="grid gap-3 lg:grid-cols-3">{milestones.map((milestone) => { const state = formatMilestoneState(standing, claimedRewardIds.has(milestone.id), milestone.repRequired); return <div key={milestone.id} className="rounded-xl p-4" style={{ background: state === 'claimed' ? '#052e16' : '#f1f5f955', border: `1px solid ${state === 'claimable' ? `${definition.color}66` : state === 'claimed' ? '#22c55e44' : '#cbd5e1'}` }}><div className="flex items-start justify-between gap-3"><div><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: state === 'locked' ? '#6b7280' : definition.color }}>{milestone.badgeLabel}</p><p className="mt-2 text-lg font-semibold" style={{ color: '#0f172a' }}>{milestone.title}</p></div><span className="rounded-full px-2 py-1 text-[10px] uppercase tracking-widest" style={{ background: '#f1f5f9', color: '#475569' }}>{milestone.repRequired} rep</span></div><p className="mt-3 text-xs leading-5" style={{ color: '#94a3b8' }}>{milestone.perkPreview}</p><p className="mt-2 text-xs leading-5" style={{ color: '#6b7280' }}>{milestone.contractHook}</p><p className="mt-3 text-sm" style={{ color: '#1e293b' }}>Reward: ${milestone.reward.cash.toLocaleString()}{milestone.reward.resourceReward ? ` + ${milestone.reward.resourceReward.amount} ${milestone.reward.resourceReward.kind}` : ''}</p><motion.button whileHover={{ scale: state === 'claimable' ? 1.02 : 1 }} whileTap={{ scale: state === 'claimable' ? 0.98 : 1 }} onClick={() => claimFactionReward(factionId, milestone.repRequired)} disabled={state !== 'claimable'} className="mt-4 w-full rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: state === 'claimable' ? `${definition.color}18` : '#f1f5f9', border: `1px solid ${state === 'claimable' ? `${definition.color}55` : '#cbd5e1'}`, color: state === 'claimed' ? '#86efac' : state === 'claimable' ? definition.color : '#6b7280' }}>{state === 'claimed' ? 'Claimed' : state === 'claimable' ? `Claim ${milestone.title}` : 'Locked'}</motion.button></div>; })}</div>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="space-y-6"><section className="rounded-2xl p-5" aria-label="reward-history" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}><div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Reward History</h3><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Claimed faction unlocks and long-term progression rewards.</p></div><p className="text-xs" style={{ color: '#94a3b8' }}>{factionRewardHistory.length} claimed</p></div><div className="mt-4 space-y-3">{factionRewardHistory.length === 0 && <div className="rounded-xl p-4 text-sm" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#94a3b8' }}>No faction milestones claimed yet. Push one crew past 20 rep to start banking explicit rewards.</div>}{factionRewardHistory.map((entry) => { const definition = FACTION_DEFINITIONS[entry.factionId]; return <div key={entry.id} className="rounded-xl p-4" style={{ background: '#f1f5f955', border: `1px solid ${definition.color}30` }}><div className="flex items-start justify-between gap-3"><div><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: definition.color }}>{entry.badgeLabel}</p><p className="mt-2 text-sm font-semibold" style={{ color: '#0f172a' }}>{entry.title}</p></div><span className="text-[11px]" style={{ color: '#6b7280' }}>{new Date(entry.claimedAt).toLocaleDateString()}</span></div><p className="mt-2 text-xs leading-5" style={{ color: '#94a3b8' }}>{entry.summary}</p></div>; })}</div></section><section className="rounded-2xl p-5" aria-label="mission-leaderboards" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Leaderboards</h3><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Global mission leaders, Sunday weekly resets, and your crew comparison with Gold/Silver/Bronze tiers.</p></div><p className="text-[11px] uppercase tracking-widest" style={{ color: '#94a3b8' }}>Rewards claimed: ${(missionRewardTotal + factionRewardCashTotal).toLocaleString()}</p></div><div className="mt-4 flex flex-wrap gap-2">{(['global', 'weekly', 'friends'] as LeaderboardMode[]).map((mode) => <button key={mode} onClick={() => setLeaderboardMode(mode)} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: leaderboardMode === mode ? '#0f766e12' : '#f1f5f9', border: `1px solid ${leaderboardMode === mode ? '#0f766e40' : '#cbd5e1'}`, color: leaderboardMode === mode ? '#0f766e' : '#94a3b8' }}>{LEADERBOARD_LABELS[mode]}</button>)}</div><div className="mt-4 rounded-xl" style={{ background: '#0b1120', border: '1px solid #172033' }}><div className="grid grid-cols-[56px_minmax(0,1fr)_100px_120px] gap-3 px-4 py-3 text-[11px] uppercase tracking-widest" style={{ color: '#64748b' }}><div>Rank</div><div>Runner</div><div className="text-right">Missions</div><div className="text-right">Rewards</div></div><div className="max-h-[560px] overflow-y-auto">{activeLeaderboard.map((entry, index) => { const rank = index + 1; const tier = getTierAccent(rank); return <div key={`${leaderboardMode}-${entry.name}-${rank}`} className="grid grid-cols-[56px_minmax(0,1fr)_100px_120px] gap-3 px-4 py-3 text-sm" style={{ borderTop: index === 0 ? 'none' : '1px solid #172033', background: entry.scope === 'You' ? '#0f766e0a' : 'transparent' }}><div><span className="inline-flex rounded-full px-2 py-1 text-[10px] uppercase tracking-widest" style={{ background: `${tier.color}18`, color: tier.color }}>#{rank}</span></div><div className="min-w-0"><p className="truncate font-medium" style={{ color: entry.scope === 'You' ? '#0f766e' : '#f8fafc' }}>{entry.name}</p><p className="mt-1 text-[11px] uppercase tracking-widest" style={{ color: '#6b7280' }}>{entry.scope} · {tier.label}</p></div><div className="text-right" style={{ color: '#e2e8f0' }}>{entry.missionsCompleted}</div><div className="text-right" style={{ color: '#475569' }}>${entry.rewardsClaimed.toLocaleString()}</div></div>; })}</div></div></section></div>
      </div>
    </div>
  );
}
...
export default function GuildPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-widest uppercase" style={{ color: '#0f766e' }}>Guild Command</h1>
        ...
        <div className="flex gap-2">
          {(['guild', 'factions'] as GuildTab[]).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: activeTab === tab ? '#0f766e12' : '#f1f5f9', border: `1px solid ${activeTab === tab ? '#0f766e40' : '#cbd5e1'}`, color: activeTab === tab ? '#0f766e' : '#94a3b8' }}>{TAB_LABELS[tab]}</button>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-6" aria-label="guild-ops">
          {guild.membershipStatus !== 'member' ? (
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
              <section className="rounded-2xl p-5" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}>
                <h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Create Guild</h2>
                ...
                <div className="mt-4 space-y-3">
                  <input aria-label="guild-name" value={guildName} onChange={(event) => setGuildName(event.target.value)} placeholder="Guild name" className="w-full rounded-lg px-3 py-2 text-sm" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }} />
                  <input aria-label="guild-tag" value={guildTag} onChange={(event) => setGuildTag(event.target.value.toUpperCase())} placeholder="Tag" className="w-full rounded-lg px-3 py-2 text-sm" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }} />
                  <div className="flex gap-2">{(['application', 'invite_only'] as GuildJoinMode[]).map((mode) => <button key={mode} onClick={() => setGuildJoinMode(mode)} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: guildJoinMode === mode ? '#0f766e12' : '#f1f5f9', border: `1px solid ${guildJoinMode === mode ? '#0f766e40' : '#cbd5e1'}`, color: guildJoinMode === mode ? '#0f766e' : '#94a3b8' }}>{mode === 'application' ? 'Application' : 'Invite Only'}</button>)}</div>
              ...
              <section className="rounded-2xl p-5" aria-label="guild-directory" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}>
                <h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Guild Directory</h2>
                <p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Apply to open guilds or accept an invite from closed crews.</p>
                <div className="mt-4 space-y-3">{guild.availableGuilds.map((entry) => <div key={entry.id} className="rounded-xl p-4" style={{ background: '#f1f5f955', border: '1px solid #cbd5e1' }}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold" style={{ color: '#0f172a' }}>{entry.name} [{entry.tag}]</p><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Lvl {entry.level} · {entry.members} members · {entry.territory.map((district) => DISTRICTS[district].name).join(', ')}</p></div><span className="rounded-full px-2 py-1 text-[10px] uppercase tracking-widest" style={{ background: entry.joinMode === 'invite_only' ? '#7c2d12' : '#f1f5f9', color: entry.joinMode === 'invite_only' ? '#fdba74' : '#93c5fd' }}>{entry.joinMode === 'invite_only' ? 'Invite' : 'Apply'}</span></div><p className="mt-3 text-xs leading-5" style={{ color: '#94a3b8' }}>{entry.description}</p><button onClick={() => joinGuild(entry.id)} className="mt-4 rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }}>{entry.joinMode === 'invite_only' ? 'Accept Invite' : 'Apply & Join'}</button></div>)}</div>
              </section>
            </div>
          ) : (
            <>
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-xl p-4" style={{ background: '#f8fafc', border: '1px solid #0f766e35' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#0f766eb0' }}>Guild Info</p><p className="mt-2 text-2xl font-semibold" style={{ color: '#0f172a' }}>{guild.name} [{guild.tag}]</p><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Level {guild.level} · Prestige {guild.prestige}</p></div>
                <div className="rounded-xl p-4" style={{ background: '#f1f5f9', border: '1px solid #60a5fa35' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#93c5fd' }}>Treasury</p><p className="mt-2 text-2xl font-semibold" style={{ color: '#0f172a' }}>${guild.treasury.toLocaleString()}</p><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>${guild.treasuryCapacity.toLocaleString()} cap · tax {Math.round(guild.taxRate * 100)}%</p></div>
                <div className="rounded-xl p-4" style={{ background: '#f1f5f955', border: '1px solid #f9731635' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#fdba74' }}>Territory</p><p className="mt-2 text-2xl font-semibold" style={{ color: '#0f172a' }}>{territoryControl}%</p><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>{guild.territory.map((district) => DISTRICTS[district].name).join(', ') || 'No claimed districts'}</p></div>
                <div className="rounded-xl p-4" style={{ background: '#f1f5f9', border: '1px solid #22c55e35' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#86efac' }}>Weekly Quest</p><p className="mt-2 text-2xl font-semibold" style={{ color: '#0f172a' }}>{guild.weeklyQuest.status === 'claimable' ? 'Ready' : 'Active'}</p><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Resets in {formatResetLabel(guild.weeklyQuest.resetsAt)}</p></div>
              </div>

              <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
                <div className="space-y-6">
                  <section className="rounded-2xl p-5" aria-label="guild-info-card" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}>
                    <div className="flex items-start justify-between gap-4"><div><h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Guild Ops</h2><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Creation, perks, upgrades, and owner-side settings all live here.</p></div>{isOwner && <button onClick={() => setShowSettings(true)} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }}>Guild Settings</button>}</div>
                    <div className="mt-4 grid gap-4 md:grid-cols-2">
                      <div className="rounded-xl p-4" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#6b7280' }}>Perks</p><div className="mt-3 space-y-2 text-sm"><p style={{ color: '#0f172a' }}>Market discount: {Math.round(guildDiscountRate * 100)}%</p><p style={{ color: '#0f172a' }}>Scavenge yield: +{Math.round(trainingRate * 100)}%</p><p style={{ color: '#0f172a' }}>Vault capacity: {guild.vault.length}/{vaultCapacity}</p><p style={{ color: '#0f172a' }}>Guild Hall: {guild.guildHallUnlocked ? 'Unlocked' : 'Locked until Lvl 5 + $50k'}</p></div></div>
                      <div className="rounded-xl p-4" style={{ background: '#f1f5f955', border: '1px solid #cbd5e1' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#6b7280' }}>Quick Donate</p><div className="mt-3 flex flex-wrap gap-2">{[5, 10, 50, 100].map((amount) => <button key={amount} onClick={() => depositGuildTreasury(amount)} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#14532d', border: '1px solid #22c55e55', color: '#bbf7d0' }}>${amount}</button>)}</div><button onClick={() => depositGuildTreasury(1000)} className="mt-3 w-full rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }}>Donate $1,000</button>{(isOwner || isOfficer) && <button onClick={() => withdrawGuildTreasury(500)} className="mt-3 w-full rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#450a0a', border: '1px solid #7f1d1d', color: '#fecaca' }}>Withdraw $500</button>}</div>
                    </div>
                  </section>

                  <section className="rounded-2xl p-5" aria-label="guild-members-table" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}>
                    <div className="flex items-center justify-between gap-3"><div><h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Members</h2><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Hierarchy, contribution, online hours, and last login are all surfaced here.</p></div><p className="text-xs" style={{ color: '#94a3b8' }}>{guild.members.length}/{guild.memberSlots}</p></div>
                    <div className="mt-4 space-y-2"><div className="grid grid-cols-[minmax(0,1.2fr)_110px_110px_100px_110px] gap-3 px-3 text-[11px] uppercase tracking-widest" style={{ color: '#64748b' }}><div>Player</div><div>Role</div><div className="text-right">Contribution</div><div className="text-right">Hours</div><div className="text-right">Last Login</div></div>{guild.members.map((member) => <div key={member.id} className="grid grid-cols-[minmax(0,1.2fr)_110px_110px_100px_110px] gap-3 rounded-xl px-3 py-3 text-sm" style={{ background: member.id === 'guild-player' ? '#0f766e0a' : '#f1f5f955', border: `1px solid ${member.id === 'guild-player' ? '#0f766e26' : '#cbd5e1'}` }}><div className="min-w-0"><p className="truncate font-medium" style={{ color: member.id === 'guild-player' ? '#0f766e' : '#f8fafc' }}>{member.name}</p><p className="mt-1 text-[11px] uppercase tracking-widest" style={{ color: member.online ? '#86efac' : '#6b7280' }}>{member.online ? 'Online' : 'Offline'}</p></div><div style={{ color: '#475569' }}>{member.role}</div><div className="text-right" style={{ color: '#e2e8f0' }}>${member.contribution.toLocaleString()}</div><div className="text-right" style={{ color: '#475569' }}>{formatHours(member.hoursOnline)}</div><div className="text-right" style={{ color: '#94a3b8' }}>{new Date(member.lastLoginAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div></div>)}</div>
                    {canManageMembers && <div className="mt-4 flex flex-wrap gap-2">{guild.members.filter((member) => member.id !== 'guild-player').map((member) => <button key={member.id} onClick={() => promoteGuildMember(member.id, member.role === 'member' ? 'officer' : 'member')} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }}>{member.role === 'member' ? `Promote ${member.name}` : `Demote ${member.name}`}</button>)}</div>}
                  </section>

                  <section className="rounded-2xl p-5" aria-label="guild-upgrades" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}>
                    <div className="flex items-center justify-between gap-3"><div><h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Guild Upgrades</h2><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Treasury capacity, training grounds, vault security, and the Guild Hall gate.</p></div>{!guild.guildHallUnlocked && <button onClick={() => unlockGuildHall()} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#7c2d12', border: '1px solid #f9731655', color: '#fdba74' }}>Unlock Guild Hall</button>}</div>
                    <div className="mt-4 grid gap-3 md:grid-cols-3">{(['treasury_capacity', 'training_grounds', 'vault_security'] as const).map((track) => <div key={track} className="rounded-xl p-4" style={{ background: '#f1f5f955', border: '1px solid #cbd5e1' }}><p className="text-[11px] uppercase tracking-[0.28em]" style={{ color: '#6b7280' }}>{track.replace('_', ' ')}</p><p className="mt-2 text-2xl font-semibold" style={{ color: '#0f172a' }}>Tier {guild.upgrades[track]}</p><button onClick={() => upgradeGuildTrack(track)} className="mt-4 w-full rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }}>Upgrade</button></div>)}</div>
                  </section>
                </div>

                <div className="space-y-6">
                  <section className="rounded-2xl p-5" aria-label="guild-weekly-quest" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}><h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Weekly Guild Quest</h2><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>{guild.weeklyQuest.description}</p><div className="mt-4 space-y-3"><div><div className="flex items-center justify-between text-xs" style={{ color: '#94a3b8' }}><span>Scavenged value</span><span>${guild.weeklyQuest.scavengedValueProgress.toLocaleString()} / ${guild.weeklyQuest.scavengedValueTarget.toLocaleString()}</span></div><div className="mt-2 h-2 rounded-full" style={{ background: '#f1f5f9' }}><div className="h-full rounded-full" style={{ width: `${Math.min(100, (guild.weeklyQuest.scavengedValueProgress / guild.weeklyQuest.scavengedValueTarget) * 100)}%`, background: '#22c55e' }} /></div></div><div><div className="flex items-center justify-between text-xs" style={{ color: '#94a3b8' }}><span>Recycled weight</span><span>{guild.weeklyQuest.recycledWeightProgress} / {guild.weeklyQuest.recycledWeightTarget} kg</span></div><div className="mt-2 h-2 rounded-full" style={{ background: '#f1f5f9' }}><div className="h-full rounded-full" style={{ width: `${Math.min(100, (guild.weeklyQuest.recycledWeightProgress / guild.weeklyQuest.recycledWeightTarget) * 100)}%`, background: '#60a5fa' }} /></div></div><button onClick={() => claimGuildWeeklyQuest()} className="w-full rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: guild.weeklyQuest.status === 'claimable' ? '#14532d' : '#f1f5f9', border: `1px solid ${guild.weeklyQuest.status === 'claimable' ? '#22c55e55' : '#cbd5e1'}`, color: guild.weeklyQuest.status === 'claimable' ? '#bbf7d0' : '#6b7280' }}>Claim Weekly Reward</button></div></section>

                  <section className="rounded-2xl p-5" aria-label="guild-war" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}><h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Guild Wars</h2><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Declare wars, hold districts for 72 hours, and cash in territory bonuses or eat a week-long penalty.</p><div className="mt-4 space-y-3"><input aria-label="war-opponent" value={warOpponent} onChange={(event) => setWarOpponent(event.target.value)} className="w-full rounded-lg px-3 py-2 text-sm" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }} /><div className="flex flex-wrap gap-2">{(['slums', 'tech', 'harbor'] as District[]).map((district) => <button key={district} onClick={() => setWarDistricts((current) => current.includes(district) ? current.filter((entry) => entry !== district) : [...current, district].slice(0, 2))} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: warDistricts.includes(district) ? '#7c2d12' : '#f1f5f9', border: `1px solid ${warDistricts.includes(district) ? '#f9731655' : '#cbd5e1'}`, color: warDistricts.includes(district) ? '#fdba74' : '#94a3b8' }}>{DISTRICTS[district].name}</button>)}</div><button onClick={() => declareGuildWar(warOpponent, warDistricts)} className="w-full rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#450a0a', border: '1px solid #7f1d1d', color: '#fecaca' }}>Declare War</button>{guild.war.status !== 'peace' && <div className="rounded-xl p-4" style={{ background: '#f1f5f955', border: '1px solid #cbd5e1' }}><p className="text-sm font-semibold" style={{ color: '#0f172a' }}>{guild.war.opponent}</p><p className="mt-1 text-xs" style={{ color: '#94a3b8' }}>Status: {guild.war.status} · districts {guild.war.targetDistricts.map((district) => DISTRICTS[district].name).join(', ') || 'none'}</p><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>{guild.war.occupationEndsAt ? `Occupation ends in ${formatResetLabel(guild.war.occupationEndsAt)}` : 'Waiting for orders'}</p><div className="mt-3 flex gap-2"><button onClick={() => resolveGuildWar('won')} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#14532d', border: '1px solid #22c55e55', color: '#bbf7d0' }}>Resolve Win</button><button onClick={() => resolveGuildWar('lost')} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#450a0a', border: '1px solid #7f1d1d', color: '#fecaca' }}>Resolve Loss</button></div></div>}</div></section>

                  <section className="rounded-2xl p-5" aria-label="guild-social" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}><div className="grid gap-6 xl:grid-cols-2"><div><h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Activity</h2><div className="mt-4 max-h-[280px] space-y-3 overflow-y-auto">{guild.activityLog.map((entry) => <div key={entry.id} className="flex gap-3 text-sm"><span>{entry.icon}</span><div><p style={{ color: '#334155' }}>{entry.text}</p><p className="text-[11px]" style={{ color: '#6b7280' }}>{new Date(entry.createdAt).toLocaleString()}</p></div></div>)}</div></div><div><h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Guild Chat</h2><div className="mt-4 max-h-[180px] space-y-3 overflow-y-auto">{guild.chatMessages.map((entry) => <div key={entry.id} className="rounded-lg p-3" style={{ background: '#f1f5f955', border: '1px solid #cbd5e1' }}><p className="text-sm font-medium" style={{ color: '#0f172a' }}>{entry.author}</p><p className="mt-1 text-sm" style={{ color: '#475569' }}>{entry.message}</p></div>)}</div><div className="mt-3 flex gap-2"><input aria-label="guild-chat-input" value={chatDraft} onChange={(event) => setChatDraft(event.target.value)} className="flex-1 rounded-lg px-3 py-2 text-sm" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }} /><button onClick={() => { postGuildChat(chatDraft); setChatDraft(''); }} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }}>Send</button></div></div></div><div className="mt-6 grid gap-6 xl:grid-cols-2"><div><h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Bulletin Board</h2>{canManageBulletins && <div className="mt-3 space-y-2"><input aria-label="guild-bulletin-title" value={bulletinTitle} onChange={(event) => setBulletinTitle(event.target.value)} className="w-full rounded-lg px-3 py-2 text-sm" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }} /><textarea aria-label="guild-bulletin-body" value={bulletinBody} onChange={(event) => setBulletinBody(event.target.value)} className="w-full rounded-lg px-3 py-2 text-sm" rows={3} style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }} /><button onClick={() => { postGuildBulletin(bulletinTitle, bulletinBody); setBulletinTitle(''); setBulletinBody(''); }} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }}>Post Bulletin</button></div>}<div className="mt-4 space-y-3">{guild.bulletinPosts.map((post) => <div key={post.id} className="rounded-lg p-3" style={{ background: '#f1f5f955', border: '1px solid #cbd5e1' }}><p className="text-sm font-semibold" style={{ color: '#0f172a' }}>{post.title}</p><p className="mt-1 text-xs" style={{ color: '#94a3b8' }}>by {post.author}</p><p className="mt-2 text-sm" style={{ color: '#475569' }}>{post.body}</p></div>)}</div></div><div><h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Guild Vault</h2><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>{guild.vault.length}/{vaultCapacity} filled</p>{canManageVault && eligibleInventory[0] && <button onClick={() => depositGuildVaultItem(eligibleInventory[0].id, 1)} className="mt-3 rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }}>Deposit 1x {eligibleInventory[0].name}</button>}<div className="mt-4 space-y-3">{guild.vault.map((entry) => <div key={entry.id} className="rounded-lg p-3" style={{ background: '#f1f5f955', border: '1px solid #cbd5e1' }}><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold" style={{ color: '#0f172a' }}>{entry.name}</p><p className="mt-1 text-xs" style={{ color: '#94a3b8' }}>{entry.quantity}x · by {entry.depositedBy}</p></div><button onClick={() => withdrawGuildVaultItem(entry.id, 1)} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }}>Withdraw</button></div></div>)}</div></div></div></section>
                </div>
              </div>

              {showSettings && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/20 p-4"><div className="w-full max-w-lg rounded-2xl p-5" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}><div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-semibold uppercase tracking-[0.28em]" style={{ color: '#1e293b' }}>Guild Settings</h2><p className="mt-1 text-xs" style={{ color: '#6b7280' }}>Owner controls for tax, member slots, and current war status.</p></div><button onClick={() => setShowSettings(false)} className="rounded-lg px-3 py-2 text-xs uppercase tracking-widest" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#0f172a' }}>Close</button></div><div className="mt-4 space-y-4"><label className="block text-xs uppercase tracking-widest" style={{ color: '#94a3b8' }}>Tax Rate {Math.round(guild.taxRate * 100)}%<input aria-label="guild-tax-rate" type="range" min={0} max={20} value={Math.round(guild.taxRate * 100)} onChange={(event) => setGuildTaxRate(Number(event.target.value) / 100)} className="mt-2 w-full" /></label><label className="block text-xs uppercase tracking-widest" style={{ color: '#94a3b8' }}>Member Slots {guild.memberSlots}<input aria-label="guild-member-slots" type="range" min={guild.members.length} max={40} value={guild.memberSlots} onChange={(event) => setGuildMemberSlots(Number(event.target.value))} className="mt-2 w-full" /></label><div className="rounded-xl p-4" style={{ background: '#f1f5f955', border: '1px solid #cbd5e1' }}><p className="text-xs uppercase tracking-widest" style={{ color: '#6b7280' }}>War Status</p><p className="mt-2 text-sm" style={{ color: '#0f172a' }}>{guild.war.status}</p><p className="mt-1 text-xs" style={{ color: '#94a3b8' }}>{guild.war.opponent ? `Opponent: ${guild.war.opponent}` : 'No active rival declared.'}</p></div><div className="rounded-xl p-4" style={{ background: '#f1f5f955', border: '1px solid #cbd5e1' }}><p className="text-xs uppercase tracking-widest" style={{ color: '#6b7280' }}>Role Permissions</p><div className="mt-3 space-y-3">{(['owner', 'officer', 'member'] as GuildRole[]).map((role) => <div key={role}><p className="text-xs uppercase tracking-widest" style={{ color: '#0f172a' }}>{role}</p><div className="mt-2 flex flex-wrap gap-2">{(['manage_members', 'withdraw_treasury', 'manage_settings', 'start_wars', 'manage_bulletin', 'manage_vault'] as GuildPermissionKey[]).map((permission) => <button key={`${role}-${permission}`} onClick={() => toggleGuildPermission(role, permission)} className="rounded-lg px-2 py-1 text-[10px] uppercase tracking-widest" style={{ background: guild.permissionsByRole[role].includes(permission) ? '#14532d' : '#f1f5f9', border: `1px solid ${guild.permissionsByRole[role].includes(permission) ? '#22c55e55' : '#cbd5e1'}`, color: guild.permissionsByRole[role].includes(permission) ? '#bbf7d0' : '#94a3b8' }}>{permission.replace('_', ' ')}</button>)}</div></div>)}</div></div></div></div></div>}
            </>
          )}
        </div>
      )}
